...
public class SSA
{
    public void tau_adaptive(Model model, float tau = 1)
    {
        ...
        foreach (State entry in model.StateDictionary.Values)
        {
            entry.UpdatePopulation(entry.NewValue);
            //entry.Value.UpdateValue(entry.Value.NewValue);
            //Debug.Log(entry.Value.Name + ": " + entry.Value.NewValue);
        }
